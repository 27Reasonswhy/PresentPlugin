package present.me.reasonswhy.presents.Me.reasonswhy.utils;
...
public class PresentMaker {
    ...
    public static ItemStack createPresent(List<ItemStack> items, String from, String to, String message) throws IOException {
        ...
        container.set(new NamespacedKey(Elf.getPlugin(), "presents"), PersistentDataType.STRING, encodedString);

        System.out.println("dumbadumbdumb: " + to);

        if (to != null){
            container.set(new NamespacedKey(Elf.getPlugin(), "presentRecipient"), PersistentDataType.STRING, to);
            lore.add(ColorUtils.translateColorCodes("&a&lTO: &c&l" + to));
            meta.setLore(lore);
        }

        if (message != null){
            container.set(new NamespacedKey(Elf.getPlugin(), "presentMessage"), PersistentDataType.STRING, message);
        }
